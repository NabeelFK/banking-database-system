...
export default function CustomerDepositPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const data = await apiFetch("/api/customer/deposit", {
        method: "POST",
        body: JSON.stringify({ accountId: Number(selectedId), amount: parsed }),
      });
      if (data.error) { setError(data.error); return; }
      setSuccess(`Deposit successful! New balance: ${fmt(data.newBalance)}`);
      setAmount("");
      // refresh accounts
      const stored = localStorage.getItem("user");
      const userId = stored ? JSON.parse(stored).userId : null;
  ...
  return (
    <CustomerPortal
      activePage="accounts"
      title="Deposit money"
      description="Add funds to one of your active bank accounts."
    >
      <div className="mx-auto max-w-lg">
        <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
          <h2 className="text-2xl font-semibold tracking-tight">Make a deposit</h2>
          <p className="mt-2 text-sm text-slate-600">Funds are added immediately to your account balance.</p>
          ...
          {loading ? (
            ...
            <p className="mt-6 text-sm text-slate-500">No active accounts available for deposit.</p>
          ) : (
              ...
              <div>
                ...
                {selectedAccount && (
                  <p className="mt-2 text-xs text-slate-500">
                    Current balance: {fmt(selectedAccount.Balance)}
                  </p>
                )}
              </div>
